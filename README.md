...
- Prompt、Image、Qwen Editノード
- Promptだけを接続した画像生成と、生成ノード1つにつき最大3枚の参照画像を使う画像編集
- PNG / JPEG / WebP / BMPの選択・ドラッグ＆ドロップ読み込み
- 元画像の縦横比に合わせたImageノード表示と解像度表示
- 設定解像度または生成結果の縦横比に合わせたQwen Editノード表示
- Input 1の解像度を生成サイズへ反映
- width、height、seed、steps、CFGのワークフロー上書きとseedランダム化ボタン
- Qwen Editの生成結果を次の生成ノードへ接続
- 生成開始から結果保存または失敗までの経過時間をQwen Editノードへ表示・保存
- 生成中のCancelボタンによるComfyUI promptの中断
- 生成結果を画像としてクリップボードへコピー、または保存先を選んで書き出し
- 生成結果をクリックして画面内へ拡大表示
- ルートフォルダと複数セッションの作成・名前変更・複製・削除
...
2. `+`からセッションを作成します。
3. ツールバー、またはキャンバスを右クリックしてImageノードを追加し、画像を選択またはドロップします。
4. Promptノードを追加し、編集内容を入力します。
5. Qwen Editノードを追加します。
6. Promptノードを`Prompt`へ、Imageノードを`Image 1`～`Image 3`へ接続します。
7. 必要に応じて生成パラメータを設定します。
8. `Generate`を押して生成します。
9. 生成結果の出力ピンを別のQwen Editノードへ接続すると、続けて再編集できます。

生成中は`Generate`ボタンが`Cancel`へ切り替わります。キャンセルするとComfyUIの対象promptを停止し、キャンセルまでの時間をノードへ残します。

生成結果をクリックすると画面内へ拡大表示します。画面外の背景をクリックするか`Esc`を押すと元の表示へ戻ります。右上にあるコピーアイコンで画像をクリップボードへコピーし、保存アイコンで任意の場所へ書き出せます。

Image 1と同じ出力サイズにする場合は、画像を接続してから`Input 1の幅・高さに合わせる`を押します。4096px以内を維持し、各辺を8の倍数へ補正します。
...
| `Ctrl+V` | コピーしたノードとエッジを同じセッションへ貼り付け |

文字入力欄へフォーカスしている間は、キャンバス用のキーボードショートカットは動作しません。

ノードのコピー内容はアプリ内に保持されます。画像ノードや生成結果を貼り付けた場合は同じセッションの画像アセットを参照するため、別セッションへの貼り付けはできません。
...
| Qwen Edit | 最大3枚の画像とPromptから画像を生成 | Prompt入力、IMAGE入力×3、IMAGE出力 |

Qwen Editノードでは次の値を指定できます。
