...
- アンドゥ・リドゥ(`Ctrl+Z` / `Ctrl+Y`)を追加した。グラフ編集を約400ms単位で最大50件履歴化し、生成の実行状態は対象外とする。履歴が参照する画像はセッション保存時の自動削除から保護する。
...
- Batch Image Generateノードで一括処理開始からの経過時間をリアルタイム表示し、完了・失敗・キャンセル後は確定した処理時間を保持するようにした。

- セッションごとのキャンバス位置とズームを`session.json`へ自動保存し、セッション切り替え・再起動・複製後に復元するようにした。保存値がない既存セッションは全体表示にフォールバックする。

- 同一セッション内のノード貼り付けで、コピー対象外の既存ノードへつながる境界エッジも維持するようにした。別セッション、削除済みの相手、または接続先の同一入力ピンが使用済みの場合は境界エッジを除外する。
