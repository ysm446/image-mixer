...
- ノード追加操作はキャンバスの右クリックメニューへ一本化する。上部バーにはLocal LLMのモデル選択ボタン、アイコン式Load／Eject操作、ComfyUI状態、Local LLM設定ボタンを置き、アプリ名や説明文、新規作成ボタンは置かない。モデル選択はポップアップ一覧で行い、ロード済み・アンロード・切替中・再ロード待ちをバーの明るさと配色で判別できるようにする。モデルバーとComfyUIボタンは説明用の副題を持たない単一行表示とし、高さを揃える。
...
- 左サイドバーでルートフォルダとセッション一覧を管理し、セッションの作成・名前変更・複製・切り替え・削除と編集内容の自動保存を行う。一覧のスクロールバーは細く簡素にし、三点メニューはスクロール領域に切られない画面基準の位置へ表示する。
- セッション複製では`assets`を新しいUUIDフォルダへコピーし、snapshot内の画像パスを複製先へ付け替える。
- ファイル選択とドラッグ＆ドロップのどちらからでも、画像を現在のセッションへimportする。
- キャンバスの空白部分へ画像をドロップした場合は、ドロップ位置へImageノードを作成して取り込んだ画像を割り当てる。
- ImageノードとImage Generateノードは共通の360 × 360px基準で画像表示サイズを計算し、同じ解像度・縦横比の画像を同じ大きさで表示する。Imageノードは画像の縦横比に合わせて表示し、解像度を表示する。未読込時は画像領域のクリックで選択し、読込後は画像クリックで拡大、右上の変更アイコンで差し替える。ドラッグ＆ドロップは常に読み込み・差し替えとして扱う。生成ノードではチェックボックスを有効にすると、Generate操作のたびにImage 1の解像度をwidth / heightへ反映する。
- Image Generateノードの結果領域とノード幅を、設定解像度または実際の生成結果の縦横比に合わせて表示する。
...
- Batch Image Generateでは開始時に選択フォルダ直下の対応画像を固定し、各画像をImage 1として既存FIFOキューへ順次投入する。処理中は開始時点からの経過時間を表示する。入力フォルダ直下へ日時付き出力フォルダを作成し、結果画像と`batch-result.json`を保存する。
