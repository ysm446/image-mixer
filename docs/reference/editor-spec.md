...
- 左 sidebar: ルートフォルダ選択、セッション一覧、新規作成、切り替え。各セッションの三点メニューから名前変更、複製、削除を行う。 三点メニューは一覧のレイアウトを押し下げず、スクロール領域に切られない画面基準の位置へ重ねて表示する。画面下端の空きが足りない場合はボタンの上側へ開く。セッション一覧のスクロールバーは細く簡素な外観にする。
